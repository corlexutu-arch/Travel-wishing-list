...
def homepage (request):
    if request.method == "POST":
        ...
        if url_receptionat:
            # 1. Trimitem URL-ul către convertapp pentru descărcare și OCR (video se șterge automat la finalul funcției)
            date_brute = proceseaza_video_si_descriere(url_receptionat)
            
            # 2. Trimitem textele extrase către extractapp pentru ca Gemini să le transforme în JSON structurat
            json_ai = extrage_date_calatorie(
                descriere_text=date_brute["descriere_text"], 
                ecran_text=date_brute["ecran_text"]
            )
            
            # 3. Salvăm rezultatul final în baza de date din mainapp
            ...
            # După salvare, trimitem utilizatorul direct pe pagina cu tabelul/lista
...
def thelist (request):
    ...
    # Trimitem datele către template-ul HTML pentru a fi afișate în tabel
